...
scan_codeql_actions() {
    ...
    echo "Analyzing..."
    gh codeql database analyze /tmp/spoolbuddy-codeql-actions \
...
scan_trivy_image() {
    ...
    docker build -t spoolbuddy:security-scan . 2>&1
    echo ""
    trivy image --severity CRITICAL,HIGH,MEDIUM spoolbuddy:security-scan 2>&1
}
...
echo -e "${BOLD}SpoolBuddy Security Scanner${NC}"
