#!/bin/bash
#
# Docker Test Suite for SpoolStation
...
print_header "SpoolStation Docker Test Suite"
...
if [ "$RUN_BUILD" = true ]; then
    ...
    if sudo docker build -t spoolbuddy:test . --pull --no-cache; then
...
if [ "$RUN_BACKEND" = true ]; then
    ...
    if sudo docker compose -f docker-compose.test.yml build --pull --no-cache backend-test; then
...
if [ "$RUN_FRONTEND" = true ]; then
    ...
    if sudo docker compose -f docker-compose.test.yml build --pull --no-cache frontend-test; then
...
if [ "$RUN_INTEGRATION" = true ]; then
    ...
    if ! sudo docker compose -f docker-compose.test.yml build --pull --no-cache integration; then
