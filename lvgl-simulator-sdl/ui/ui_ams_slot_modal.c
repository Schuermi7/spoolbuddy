...
// Catalog colors (from database)
#define MAX_CATALOG_COLORS 50
static ColorCatalogEntry g_catalog_colors[MAX_CATALOG_COLORS];
...
static void filter_k_profiles(void) {
    ...
    for (int i = 0; i < g_k_profile_count; i++) {
        ...
        if (i < 10 || matches_full || matches_material) {
            ESP_LOGI(TAG, "  [%d] cali_idx=%d ext=%d name='%s' -> full=%s mat=%s",
                     i, g_k_profiles[i].cali_idx, g_k_profiles[i].extruder_id,
                     g_k_profiles[i].name,
                     matches_full ? "YES" : "no",
        ...
        if (brand_material_match < 0 && matches_full) {
            ...
            ESP_LOGI(TAG, "  -> Brand+material match: idx=%d cali_idx=%d", i, g_k_profiles[i].cali_idx);
        }
    ...
    if (g_selected_k_idx >= 0) {
        ESP_LOGI(TAG, "Auto-selected K-profile: idx=%d cali_idx=%d name='%s' extruder_id=%d",
                 g_selected_k_idx, g_k_profiles[g_selected_k_idx].cali_idx,
                 g_k_profiles[g_selected_k_idx].name, g_k_profiles[g_selected_k_idx].extruder_id);
    } else {
            ...
            if (k_profile_matches(&g_k_profiles[i], brand, material, g_extruder_id)) {
                if (match_idx == selected - 1) {
                    ...
                    ESP_LOGI(TAG, "Selected K-profile idx: %d, cali_idx: %d, name: %s, k_value: %s",
                             g_selected_k_idx, g_k_profiles[i].cali_idx, g_k_profiles[i].name, g_k_profiles[i].k_value);
                    break;
                }
                match_idx++;
            }
...
static void configure_handler(lv_event_t *e) {
    ...
    char tray_color[16];
    snprintf(tray_color, sizeof(tray_color), "%sFF", color_hex);  // Add alpha
...
static void update_k_profile_dropdown(void) {
    ...
    char options[2048] = "Default (0.020)\n";
    ...
    ESP_LOGI(TAG, "K-profile dropdown: %d profiles match (extruder=%d)", match_count, g_extruder_id);
...
static void rebuild_colors_ui(void) {
    if (!g_colors_container) return;

    // Clear existing children
    lv_obj_clean(g_colors_container);

    // -------------------------------------------------------------------------
    // Catalog colors section (if any found)
    // -------------------------------------------------------------------------
    if (g_catalog_color_count > 0) {
        ...
        // Catalog colors grid (no max height - container scrolls)
        ...
        // No max_height - let parent container scroll for many colors

        for (int i = 0; i < g_catalog_color_count && i < MAX_CATALOG_COLORS; i++) {
            // Parse hex color (may have # prefix)
            const char *hex = g_catalog_colors[i].hex_color;
            if (hex[0] == '#') hex++;
            ...
            // Store hex without # for click handler
    ...
    if (g_catalog_color_count == 0) {
        ...
        // More colors button
        lv_obj_t *more_colors_row = lv_obj_create(g_colors_container);
        lv_obj_set_size(more_colors_row, 310, LV_SIZE_CONTENT);
        lv_obj_set_style_bg_opa(more_colors_row, 0, 0);
        lv_obj_set_style_border_width(more_colors_row, 0, 0);
        lv_obj_set_style_pad_all(more_colors_row, 0, 0);
        lv_obj_clear_flag(more_colors_row, LV_OBJ_FLAG_SCROLLABLE);

        lv_obj_t *more_btn = lv_button_create(more_colors_row);
        lv_obj_set_size(more_btn, 100, 32);
        lv_obj_set_style_bg_color(more_btn, lv_color_hex(0x333333), 0);
        lv_obj_set_style_radius(more_btn, 6, 0);
        lv_obj_add_event_cb(more_btn, toggle_extended_colors_handler, LV_EVENT_CLICKED, NULL);
        lv_obj_t *more_label = lv_label_create(more_btn);
        lv_label_set_text(more_label, "More Colors...");
        lv_obj_set_style_text_font(more_label, &lv_font_montserrat_12, 0);
        lv_obj_center(more_label);

        // Extended colors (if shown)
        if (g_show_extended_colors) {
            lv_obj_t *ext = lv_obj_create(g_colors_container);
            lv_obj_set_size(ext, 310, LV_SIZE_CONTENT);
            lv_obj_set_style_bg_opa(ext, 0, 0);
            lv_obj_set_style_border_width(ext, 0, 0);
            lv_obj_set_style_pad_all(ext, 0, 0);
            lv_obj_set_flex_flow(ext, LV_FLEX_FLOW_ROW_WRAP);
            lv_obj_set_style_pad_gap(ext, 6, 0);
            lv_obj_clear_flag(ext, LV_OBJ_FLAG_SCROLLABLE);

            for (int i = 0; i < EXTENDED_COLORS_COUNT; i++) {
                lv_obj_t *color_btn = lv_obj_create(ext);
                lv_obj_set_size(color_btn, 28, 28);
                lv_obj_set_style_bg_color(color_btn, lv_color_hex(hex_to_color(EXTENDED_COLORS[i].hex)), 0);
                lv_obj_set_style_bg_opa(color_btn, 255, 0);
                lv_obj_set_style_radius(color_btn, 4, 0);
                lv_obj_set_style_border_width(color_btn, 1, 0);
                lv_obj_set_style_border_color(color_btn, lv_color_hex(0x666666), 0);
                lv_obj_clear_flag(color_btn, LV_OBJ_FLAG_SCROLLABLE);
                lv_obj_add_flag(color_btn, LV_OBJ_FLAG_CLICKABLE);
                lv_obj_add_event_cb(color_btn, color_select_handler, LV_EVENT_CLICKED, (void*)EXTENDED_COLORS[i].hex);
            }
        }
    }
}
...
static void populate_preset_list(void) {
    ...
    ESP_LOGI(TAG, "populate_preset_list: rendered %d presets", rendered);
}
...
// Timer callback to load data and build content after modal shows loading state
static void load_data_timer_cb(lv_timer_t *t) {
    lv_timer_delete(t);

    if (!g_modal || !g_modal_open) return;

    // Fetch presets (thread-safe with mutex)
    ...
    // Fetch K-profiles (thread-safe with mutex)
    ...
    for (int i = 0; i < g_k_profile_count && i < 10; i++) {
        ESP_LOGI(TAG, "K-profile[%d]: cali_idx=%d extruder=%d name='%s' k='%s' filament_id='%s'",
                 i, g_k_profiles[i].cali_idx, g_k_profiles[i].extruder_id,
                 g_k_profiles[i].name, g_k_profiles[i].k_value, g_k_profiles[i].filament_id);
    }
...
void ui_ams_slot_modal_open(const char *printer_serial, int ams_id, int tray_id,
    ...
    if (tray_color && tray_color[0]) {
        ...
        lv_obj_set_size(color_swatch, 28, 28);
        ...
        lv_obj_set_style_radius(color_swatch, 14, 0);
        lv_obj_set_style_border_width(color_swatch, 1, 0);
        lv_obj_set_style_border_color(color_swatch, lv_color_hex(0x666666), 0);
    ...
    if (tray_type && tray_type[0]) {
        lv_obj_t *type_label = lv_label_create(slot_info);
        char type_text[32];
        snprintf(type_text, sizeof(type_text), "(%s)", tray_type);
        ...
        lv_obj_align(type_label, LV_ALIGN_LEFT_MID, 180, 0);
    }
...
static void build_modal_content(void) {
    ...
    lv_obj_set_size(g_k_dropdown, 310, 40);
    lv_obj_align(g_k_dropdown, LV_ALIGN_TOP_LEFT, 0, 24);
    lv_dropdown_set_options(g_k_dropdown, "Default (0.020)");
    ...
    lv_obj_set_size(g_colors_container, 310, 162);  // Max height before buttons (286-116-8=162), scrollable for many colors
    ...
    lv_obj_set_style_text_color(g_error_label, lv_color_hex(0xFF5555), 0);
    lv_obj_align(g_error_label, LV_ALIGN_BOTTOM_LEFT, 0, -52);
    lv_obj_add_flag(g_error_label, LV_OBJ_FLAG_HIDDEN);

    lv_obj_t *buttons_row = lv_obj_create(g_right_col);
    lv_obj_set_size(buttons_row, 310, 44);
    lv_obj_align(buttons_row, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_set_style_bg_opa(buttons_row, 0, 0);
    ...
    lv_obj_t *reread_btn = lv_button_create(buttons_row);
    lv_obj_set_size(reread_btn, 95, 44);
    lv_obj_set_style_bg_color(reread_btn, lv_color_hex(0x333333), 0);
    lv_obj_set_style_radius(reread_btn, 8, 0);
    lv_obj_add_event_cb(reread_btn, reread_handler, LV_EVENT_CLICKED, NULL);
    lv_obj_t *reread_label = lv_label_create(reread_btn);
    lv_label_set_text(reread_label, LV_SYMBOL_REFRESH " Re-read");
    lv_obj_set_style_text_font(reread_label, &lv_font_montserrat_14, 0);
    lv_obj_center(reread_label);

    lv_obj_t *clear_btn = lv_button_create(buttons_row);
    ...
    g_configure_btn = lv_button_create(buttons_row);
    lv_obj_set_size(g_configure_btn, 120, 44);
    lv_obj_set_style_bg_color(g_configure_btn, lv_color_hex(0x444444), 0);
    lv_obj_set_style_radius(g_configure_btn, 8, 0);
    lv_obj_add_event_cb(g_configure_btn, configure_handler, LV_EVENT_CLICKED, NULL);
    lv_obj_t *configure_label = lv_label_create(g_configure_btn);
    lv_label_set_text(configure_label, LV_SYMBOL_OK " Configure");
    lv_obj_set_style_text_font(configure_label, &lv_font_montserrat_14, 0);
    lv_obj_center(configure_label);

    update_configure_button_state();

    // Create keyboard at bottom
    g_keyboard = lv_keyboard_create(g_modal);
    lv_obj_set_size(g_keyboard, 768, KEYBOARD_HEIGHT);
    ...
    lv_obj_set_style_bg_color(g_keyboard, lv_color_hex(0x252525), 0);
    lv_obj_set_style_bg_color(g_keyboard, lv_color_hex(0x333333), LV_PART_ITEMS);
    lv_obj_set_style_bg_color(g_keyboard, lv_color_hex(0x32CD32), LV_PART_ITEMS | LV_STATE_CHECKED);
    lv_obj_set_style_text_color(g_keyboard, lv_color_hex(0xfafafa), LV_PART_ITEMS);
}
